...
export function PharmacyWorkspace() {
  ...
  return <div className="space-y-6">
    <Card className="overflow-hidden border-emerald-100 bg-gradient-to-br from-emerald-800 via-emerald-700 to-teal-500 text-white"><CardContent className="grid gap-4 p-6 lg:grid-cols-[1.4fr_repeat(3,0.45fr)]"><div><Badge className="bg-white/15 text-white">Pharmacy & formulary</Badge><h2 className="mt-3 text-2xl font-semibold">Prescribe, dispense & control drug stock</h2><p className="mt-2 text-sm text-emerald-50">Medication orders stay connected to the patient Hospital ID and clinical encounter.</p></div>{[["Medications", data?.medications.length ?? 0], ["Awaiting dispense", pending.length], ["Low stock", lowStock.length]].map(([label, value]) => <div key={label} className="rounded-2xl bg-white/10 p-4"><p className="text-xs uppercase tracking-wider text-emerald-100">{label}</p><p className="mt-2 text-3xl font-semibold">{value}</p></div>)}</CardContent></Card>
