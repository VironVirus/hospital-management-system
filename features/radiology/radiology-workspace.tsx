...
export function RadiologyWorkspace() {
  ...
  return <div className="space-y-6">
    <Card className="overflow-hidden border-violet-100 bg-gradient-to-br from-violet-800 via-indigo-700 to-sky-600 text-white"><CardContent className="grid gap-5 p-6 lg:grid-cols-[1.4fr_repeat(3,0.5fr)] lg:items-center"><div><Badge className="bg-white/15 text-white">Radiology department</Badge><h2 className="mt-3 text-2xl font-semibold">Imaging requests, workflow, reports, and billing</h2><p className="mt-2 text-sm text-violet-50">Every study is linked to the patient&apos;s Hospital ID, encounter, clinical indication, report, and account.</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs uppercase">Waiting</p><p className="mt-2 text-3xl font-semibold">{requests.filter((item) => item.status === "Requested").length}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs uppercase">In progress</p><p className="mt-2 text-3xl font-semibold">{requests.filter((item) => item.status === "In Progress").length}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs uppercase">Completed</p><p className="mt-2 text-3xl font-semibold">{requests.filter((item) => item.status === "Completed").length}</p></div></CardContent></Card>
