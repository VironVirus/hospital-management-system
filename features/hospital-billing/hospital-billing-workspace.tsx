...
export function HospitalBillingWorkspace() {
  ...
  const [ledgerGroup, setLedgerGroup] = useState<EncounterCharge["payment_status"]>("Unpaid");
  ...
  const ledgerGroups = (["Unpaid", "Partial", "Paid", "Waived"] as EncounterCharge["payment_status"][]).map((status) => ({ status, records: (data?.charges ?? []).filter((charge) => charge.payment_status === status) }));
  const visibleCharges = ledgerGroups.find((group) => group.status === ledgerGroup)?.records ?? [];
