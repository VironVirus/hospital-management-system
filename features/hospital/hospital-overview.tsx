...
function Metric({ icon: Icon, label, value, hint, tone }: { icon: LucideIcon; label: string; value: string; hint: string; tone: string }) {
  return <Card><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-slate-600">{label}</p><p className="mt-2 text-3xl font-semibold text-slate-950">{value}</p><p className="mt-2 text-xs text-slate-500">{hint}</p></div><div className={`rounded-2xl p-3 ${tone}`}><Icon className="h-5 w-5" /></div></div></CardContent></Card>;
}
...
export function HospitalOverview() {
  ...
  if (loading || overviewQuery.isLoading) return <Card><CardContent className="flex items-center gap-3 p-8 text-sm text-slate-600"><Loader2 className="h-4 w-4 animate-spin" />Loading hospital command centre...</CardContent></Card>;
  if (!facilityId) return <Card><CardHeader><CardTitle>Hospital setup required</CardTitle><CardDescription>Complete the St Gianna Specialist Hospital setup before opening the dashboard.</CardDescription></CardHeader></Card>;
  ...
  return <div className="space-y-6">
    <Card className="overflow-hidden border-sky-100 bg-[radial-gradient(circle_at_top_right,rgba(45,212,191,0.35),transparent_35%),linear-gradient(135deg,#0f4c5c,#0d7180_60%,#0e9f9a)] text-white"><CardContent className="grid gap-6 p-6 lg:grid-cols-[1.35fr_0.65fr] lg:items-end"><div><Badge className="bg-white/15 text-white">St Gianna Specialist Hospital</Badge><h2 className="mt-4 text-3xl font-semibold">Good care starts with one complete patient record.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-cyan-50">{facilityName || "St Gianna Specialist Hospital"} connects reception, doctors, nursing, wards, laboratory, radiology, pharmacy, billing, accounts, and stores around each patient&apos;s permanent Hospital ID.</p><p className="mt-2 text-xs text-cyan-100">No 6, 18 Road, Upper North, Transekulu, Enugu, Enugu State</p></div><div className="grid grid-cols-2 gap-3"><Button asChild className="bg-white text-teal-800 hover:bg-teal-50"><Link href="/clinical"><ClipboardPlus className="h-4 w-4" />New encounter</Link></Button><Button asChild variant="outline" className="border-white/30 bg-white/10 text-white hover:bg-white/20 hover:text-white"><Link href="/patients"><UserRound className="h-4 w-4" />Find patient</Link></Button></div></CardContent></Card>
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={UsersRound} label="Registered patients" value={String(data?.patients.length ?? 0)} hint="Permanent Hospital IDs" tone="bg-sky-100 text-sky-700" /><Metric icon={HeartPulse} label="Active encounters" value={String(openEncounters.length)} hint="Open and inpatient visits" tone="bg-teal-100 text-teal-700" /><Metric icon={BedDouble} label="Inpatients" value={String(data?.admissions.length ?? 0)} hint="Currently admitted" tone="bg-indigo-100 text-indigo-700" /><Metric icon={ReceiptText} label="Outstanding" value={money(outstanding)} hint="Clinical patient accounts" tone="bg-amber-100 text-amber-700" /></section>
