...
import { KeyRound, Loader2, Search, ShieldCheck, UserPlus, UsersRound } from "lucide-react";
...
type StaffDraft = { display_name: string; password: string; role: AppRole };
type CreatedStaff = {
  display_name: string;
  email: string;
  role: AppRole;
...
export function UserManagementPanel() {
  ...
  const saveStaff = async (profile: StaffProfile) => {
    const draft = drafts[profile.id] ?? { display_name: profile.display_name ?? "", password: "", role: profile.role };
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { error?: string; password_changed?: boolean } | null;
      if (!response.ok) throw new Error(payload?.error || "Staff account could not be updated.");
      ...
      await queryClient.invalidateQueries({ queryKey: ["admin", "staff"] });
      toast({ title: password ? "Staff and password updated" : "Staff updated", variant: "success" });
    } catch (error) {
        ...
        <CardContent className="space-y-4">
          ...
          {createdStaff ? (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
              <p className="font-semibold">Account ready: {createdStaff.display_name}</p>
              <p className="mt-1">
                {createdStaff.email} · {formatAppRole(createdStaff.role)}
              </p>
              {createdStaff.temporary_password ? (
                <p className="mt-2 rounded-lg bg-white px-3 py-2 font-mono">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <CardDescription>{visibleStaff.length} staff</CardDescription>
            </div>
        ...
        <CardContent className="space-y-3">
          <div className="relative">
            ...
            <Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search staff" />
          </div>
          ...
          {visibleStaff.map((profile) => {
            ...
            return (
              <div key={profile.id} className="grid gap-3 rounded-2xl border p-4 lg:grid-cols-[minmax(0,1fr)_180px_minmax(0,1fr)_auto] lg:items-end">
                <div>
                  <Label>Name</Label>
                  <Input
                    className="mt-1"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setDrafts((current) => ({
                        ...current,
                        [profile.id]: { ...draft, role: event.target.value as AppRole }
                      }))
                    }
                  >
                    {appRoles.map((item) => (
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setDrafts((current) => ({
                        ...current,
                        [profile.id]: { ...draft, password: event.target.value }
                      }))
                    }
                ...
                <Button className="w-full lg:w-auto" onClick={() => saveStaff(profile)} disabled={savingId === profile.id}>
                  {savingId === profile.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />}
                  Save
                </Button>
              </div>
            );
